...
async function authedReq(runId: string, n: string) {
  ...
  return new Request(
    `http://localhost/api/usecases/cucp-reevals/run/${runId}/level/${n}/clear`,
    { method: "POST", headers: { cookie: `govdoc_session=${token}` }, body: "{}" },
  );
}

describe("POST /api/usecases/cucp-reevals/run/[runId]/level/[n]/clear", () => {
  it("returns 401 without a session", async () => {
    const res = await POST(
      new Request("http://localhost/api/usecases/cucp-reevals/run/r/level/1/clear", { method: "POST" }),
      { params: Promise.resolve({ runId: "r", n: "1" }) },
    );
    expect(res.status).toBe(401);
  });

  it("returns 400 when n != 1", async () => {
  ...
  it("resolves the L1 waiter with action=clear (200)", async () => {
    ...
    await expect(waiter).resolves.toEqual({ action: "clear" });
  });
});
